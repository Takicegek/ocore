...
const QString BITCOIN_IPC_PREFIX("ocore:");
const char* BITCOIN_REQUEST_MIMETYPE = "application/ocore-paymentrequest";
const char* BITCOIN_PAYMENTACK_MIMETYPE = "application/ocore-paymentack";
const char* BITCOIN_PAYMENTACK_CONTENTTYPE = "application/ocore-payment";
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start ocore: click-to-pay handler"));
        }
        else {
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(uint160, qint64)& sendingTo, sendingTos) {
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
	...
	CKeyID keyID = boost::get<CKeyID>(*refundAddresses.begin());
        payments::Output* refund_to = payment.add_refund_to();
        refund_to->set_key(keyID.begin(), 20);
    }
    else {
        CPubKey newKey;
