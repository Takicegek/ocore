...
void BitcoinGUI::createActions(bool fIsTestnet)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a OCore address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ocore: URIs)"));
    ...
    if (!fIsTestnet)
        aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About OCore"), this);
    else
        aboutAction = new QAction(QIcon(":/icons/bitcoin_testnet"), tr("&About OCore"), this);
    aboutAction->setStatusTip(tr("Show information about OCore"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for OCore"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your OCore addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified OCore addresses"));
    ...
    openAction->setStatusTip(tr("Open a ocore: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setStatusTip(tr("Show the OCore help message to get a list with possible OCore command-line options"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        setNumBlocks(clientModel->getNumBlocks(), clientModel->getNumHeaders());
...
void BitcoinGUI::createTrayIcon(bool fIsTestnet)
{
    ...
    if (!fIsTestnet)
    {
        trayIcon->setToolTip(tr("OCore client"));
        trayIcon->setIcon(QIcon(":/icons/toolbar"));
    }
    else
    {
        trayIcon->setToolTip(tr("OCore client") + " " + tr("[testnet]"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to OCore network", "", count));
}

void BitcoinGUI::setNumBlocks(int blocks, int headers)
{
    ...
    QDateTime lastHeaderDate = clientModel->getLastHeaderDate();
    QDateTime currentDate = QDateTime::currentDateTime();
    int secs = lastHeaderDate.secsTo(currentDate);

    if(blocks < headers)
    {
        tooltip = tr("Processed %1 of %2 (estimated) blocks of transaction history.").arg(blocks).arg(headers);
    }
    else
    {
    ...
    progressBarLabel->setVisible(!fSynced);
    progressBar->setVisible(!fSynced);

#ifdef ENABLE_WALLET
        if(walletFrame)
            walletFrame->showOutOfSyncWarning(!fSynced);
#endif
...
{
    QString strTitle = tr("OCore"); // default title
...
bool BitcoinGUI::eventFilter(QObject *object, QEvent *event)
{
    ...
    if (event->type() == QEvent::MouseButtonPress) {
        QPoint p;
        p = progressBar->mapFromGlobal(QCursor::pos());
        // qDebug("pos: %d, %d", p.y(), event->type());

        if (p.x() >= 0 && p.y() >= 0 && p.x() < progressBar->width() && p.y() < progressBar->height() && fResyncRequired) {
	    QMessageBox::StandardButton reply;
  	    reply = QMessageBox::question(this, "Resync", "Resync to network? This will delete local blockchain copy and restart the application",
                               QMessageBox::Yes|QMessageBox::No);
  	    if (reply == QMessageBox::Yes) {
		clientModel->SystemResync();
	    }
        }
    }
