...
Value getrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
	    ...
	    "	    \"address\" : \"addr\",(string) OCore address representation of public key hash\n"
            ...
            "     {\n"
            "       \"value\" : x.xxx,     (ep) The value in OXN\n"
            "       \"n\" : n,             (numeric) index\n"
            "       \"pubkey\": \"key\",   (string) The public key hash of the input\n"
	    "	    \"address\" : \"addr\",(string) OCore address representation of public key hash\n"
            "     }\n"
...
Value listbalances(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2 || params.size() < 2)
        throw runtime_error(
            ...
            "2. \"addresses\"    (string) A json array of ocore addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) ocore address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"address\" : \"address\",  (string) the ocore address\n"
	    "    \"ours\"    : true/false,   (bool) is the account belongs to the local wallet\n"
            ...
            "    \"balance\" : x.xxx,        (ep) the account balance in OXN\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    if (params.size() > 1)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid ocore address: ")+input.get_str());
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() < 2 && params.size() > 4 ))
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (ep, required) The key is the ocore address, the value is the OXN amount\n"
            "      ,...\n"
            "    }\n"
    ...
    BOOST_FOREACH(const Pair& input, inputs)
    {
    	...
    	if (!address.IsValid())
    		throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid OCore address: ")+input.name_);
    ...
    BOOST_FOREACH(const Pair& output, outputs)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid OCore address: ")+output.name_);
