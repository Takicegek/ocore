...
bool static Bind(const CService &addr, unsigned int flags) {
    ...
    std::string strError;
    int errCnt=0;
    while(!BindListenPort(addr, strError)){
	MilliSleep(1000);
	if(errCnt > 10)
	    break;
	errCnt++;
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ocore.conf)") + "\n";
    if (hmm == HMM_BITCOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ocored.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 8253 or testnet: 18253)") + "\n";
    strUsage += "  -proxy=<ip:port>       " + _("Connect through SOCKS proxy") + "\n";
    strUsage += "  -seednode=<ip>         " + _("Connect to a node to retrieve peer addresses, and disconnect") + "\n";
    strUsage += "  -socks=<n>             " + _("Select SOCKS version for -proxy (4 or 5, default: 5)") + "\n";
    strUsage += "  -timeout=<n>           " + _("Specify connection timeout in milliseconds (default: 5000)") + "\n";
    strUsage += "  -solomine		  " + _("Allow mining to continue even if no peer connections") + "\n";
    strUsage += "  -resync		  " + _("Force the system to purge the blockchain and resync") + "\n";
    strUsage += "  -purge                 " + _("Purge all unneeded historical data from the database") + "\n";
#ifdef USE_UPNP
#if USE_UPNP
    ...
    strUsage += "\n" + _("RPC SSL options: (see the OCore Wiki for SSL setup instructions)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // backtrace on SIG_ABORT
    struct sigaction sa_abort;
    sa_abort.sa_handler = HandleSIGABORT;
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. OCore Core is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("OCore version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of OCore") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart OCore to complete") << "\n";
    ...
    LogPrintf("nBestHeight = %ld\n",                   chainActive.Height());
    LogPrintf("nBestHeaderHeight = %ld\n",             chainHeaders.Height());
#ifdef ENABLE_WALLET
